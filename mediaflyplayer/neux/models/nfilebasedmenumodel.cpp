 ...
 ****************************************************************************
 *
 * NFileBasedMenuModel implement routines.
 *
 * REVISION:
 * 
...
#include <QTextCodec>
#include <QFileInfo>
#include "nfilebasedmenumodel.h"
#include "nlistview.h"

NFileBasedMenuModel::NFileBasedMenuModel (QWidget *parent)
: QAbstractListModel(parent)
{
}

int NFileBasedMenuModel::rowCount(const QModelIndex &parent) const
{
	if(parent.isValid())
		return 0;
...
QVariant NFileBasedMenuModel::data(const QModelIndex &index, int role) const
{
	if(index.row() < 0 )
		return QVariant();
	...
	else if(role == IsSubmenuRole)
		return listEntries.at(index.row()).isSubmenu();
	else if(role == HelpIdRole)
		return listEntries.at(index.row()).helpId();
	return QVariant();
}

bool NFileBasedMenuModel :: loadFromFile(const QString &menuFilePath)
{
...
const QString & NFileBasedMenuModel::getMenuPath()
{
	return menuPath;
}

const QString & NFileBasedMenuModel::getMenuTitle()
{
	return menuTitle;
}

int NFileBasedMenuModel::getIndexOfCommand(QString command)
{
